...
#define NUM_EVENTS 1  // 只采集 dTLB-load-misses
...
const uint64_t event_ids[NUM_EVENTS] = {
    0x04 // dTLB-load-misses （需要 `perf list` 确认这个值是否正确）
};
...
int create_perf_event(int cpu, int grp_fd, int event_idx, uint64_t *ioc_id) {
    ...
    pea.config = (PERF_COUNT_HW_CACHE_DTLB | 
                  ...
                  (PERF_COUNT_HW_CACHE_RESULT_MISS << 16));  // dTLB Miss
...
int main() {
    ...
    printf("开始采集 %d 个 CPU 的 dTLB-load-misses（每 %d 秒更新一次）\n", cpu_count, SAMPLE_INTERVAL);
    ...
    while (!stop) {
        ...
        for (i = 0; i < cpu_count; i++) {
            ...
            printf("CPU %d: read() returned %zd bytes\n", i, bytes_read);

            for (k = 0; k < rf->nr; k++) {
                if (rf->values[k].id == ids[i][0]) {
                    values[i][0] = rf->values[k].value;
                }
            }
