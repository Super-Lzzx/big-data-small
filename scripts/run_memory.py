...
# -*- coding: utf-8 -*-
"""
run_mem.py —— 内存密集型场景一键采集+归因，结果保存到 stress-results/memory
"""
import subprocess
import time
import os
...
# 内存密集型场景定义：VM拷贝 + memtester（使用 timeout 避免卡死）
workloads = {
    "memory": [
        # VM 内存拷贝压力
...
for name, cmds in workloads.items():
    ...
    # 5. 停止内存负载
    for p, cmd in mem_procs:
        stop_process(p, f"内存负载 `{cmd}`", timeout=15)

    # 6. 移动并保存采集结果到 stress-results/memory/
