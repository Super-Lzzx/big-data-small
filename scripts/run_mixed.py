...
# -*- coding: utf-8 -*-
"""
run_mixed.py —— 混合负载场景一键采集+归因，结果保存到 stress-results/mixed
负载采用 stress-ng 混合模式：CPU + I/O + 内存
"""
import subprocess
import time
import os
...
for name, cmds in workloads.items():
    ...
    # 1. 清理旧文件
    for fname in ("cpu.csv", "sched.csv"):
    ...
    # 5. 停止混合负载
    for p, cmd in procs:
        stop_process(p, f"混合负载 `{cmd}`", timeout=15)

    # 6. 保存结果到 stress-results/mixed/
